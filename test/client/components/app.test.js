...
import FinalNotificationList from "../../../src/client/containers/NotificationList"
...
describe("<App />", () => {
	...
	it("<Switch /> should have Route to /lobby path", () => {
		const wrapper = shallow(<App />)
		expect(wrapper.find(Switch).find(Route).at(1).prop('path')).to.equal('/lobby')
	})
	it("<Switch /> should have Route to /:hashName path", () => {
		const wrapper = shallow(<App />)
		expect(wrapper.find(Switch).find(Route).at(2).prop('path')).to.equal('/:hashName')
	})
})
